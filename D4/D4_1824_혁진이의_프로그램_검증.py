# [아이디어]
# get_status는 문제에 제시된 조건들을 처리한다.
# search는 DFS 방식으로 진행한다.
# DFS 방식으로 진행시 진행하는 방향을 기준으로 판단하며, 해당 방향으로 진행시 범위 내이면서 방문하지 않는 경우에 진행한다.
# 여기서 방문을 확인하는 방법으로 dictionary를 사용하는데, "x좌표.y좌표.방향.메모리"로 만들어 해당 key가 존재하면 이미 방문한 것이다.
